...
type User struct {
	...
	Houses        []*House      `gorm:"-"`                            //用户发布的房屋信息  一个人多套房
...
type House struct {
	...
	Room_count      int           `gorm:"default:1" json:"room_count"` //房间数目
	...
	Facilities      []*Facility   `gorm:"many2many:houses_facilities" json:"facilities"` //房屋设施   与设施表进行关联
...
type Area struct {
	...
	Houses []*House `json:"houses" gorm:"-"`      //区域所有的房屋   与房屋表进行关联
}
...
type Facility struct {
	...
	Houses []*House `gorm:"-"`       //都有哪些房屋有此设施  与房屋表进行关联的
}
